...
- [~] Real delivery. `eas.json` added, `extra.eas.projectId` placeholder in `app.json` (replace with the real id from the Expo dashboard before `eas build`). FCM credentials and a dev build still needed; nothing below `pushTargets` has ever run. `expo-notifications` is now in `app.json` plugins
